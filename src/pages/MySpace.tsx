...
const myListings = [
  { id: '1', title: 'Ноутбук Lenovo ThinkPad', price: 12000, type: 'item' as const, status: 'active' },
  { id: '2', title: 'Переклад документів EN/UK', price: 800, type: 'service' as const, status: 'active' },
];

const myHistory = [
  { id: 'h1', date: '2024-02-14', title: 'Курс Python', amount: 3200, direction: 'Розробка дронів' },
  { id: 'h2', date: '2024-02-10', title: 'Вінтажний годинник', amount: 4500, direction: 'Гурток «Науковий»' },
  { id: 'h3', date: '2024-01-28', title: 'Консультація юриста', amount: 1200, direction: '3-тя штурмова' },
];
...
export default function MySpace({ onShowAbout }: MySpaceProps) {
  ...
  const totalContribution = myHistory.reduce((sum, h) => sum + h.amount, 0);
      ...
      <GlassCard strong className="p-5 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-xl font-bold" style={{ color: '#2A2418' }}>
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-xs" style={{ color: 'rgba(42,36,24,0.4)' }}>з лютого 2024</span>
            </div>
          </div>
        </div>
      </GlassCard>
        ...
        <div className="space-y-3">
          {myListings.length > 0 ? myListings.map(listing => (
            <GlassCard key={listing.id} className="p-4 flex items-center gap-3">
              ...
              <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: 'rgba(124,179,66,0.2)', color: '#7CB342' }}>
            ...
            <GlassCard key={item.id} className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0 pr-3">
                  <p className="font-medium text-sm" style={{ color: '#2A2418' }}>{item.title}</p>
                  <p className="text-xs mt-0.5" style={{ color: 'rgba(42,36,24,0.5)' }}>{item.direction}</p>
                  <p className="text-xs mt-0.5" style={{ color: 'rgba(42,36,24,0.4)' }}>{item.date}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-sm" style={{ color: '#7CB342' }}>₴ {item.amount.toLocaleString('uk-UA')}</p>
